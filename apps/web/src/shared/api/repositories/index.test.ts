...
describe('Repository Initialization Module', () => {
  ...
  describe('initializeRepositories', () => {
    ...
    it('should handle IndexedDB initialization errors gracefully', async () => {
      const { IndexedDBEventStore } = await import('@twsoftball/infrastructure');
      (IndexedDBEventStore as unknown as MockedFunction<() => unknown>).mockImplementation(() => {
      ...
      await expect(initializeRepositories(config)).rejects.toThrow(
