...
describe('GameAdapter', () => {
  ...
  describe('substitutePlayer', () => {
    it('should execute SubstitutePlayer use case with UI data', async () => {
      ...
      // Mock team lineup repository to return lineup data
      const mockTeamLineup = {
        id: { value: 'home-lineup-1', equals: vi.fn().mockReturnValue(true) },
        getPlayerInfo: vi.fn().mockImplementation((playerId: { value: string }) => {
        ...
        getActiveLineup: vi.fn().mockReturnValue([
          {
            ...
            currentPlayer: {
              ...
              equals: vi
                .fn()
                .mockImplementation((other: { value: string }) => other.value === 'player-1'),
            },
          },
        ]),
      };
  ...
  describe('error handling', () => {
    ...
    it('should handle null/undefined errors', async () => {
      // Mock repositories to avoid "Game not found" error and test the null rejection specifically
      const mockGame = {
        id: { value: 'game-1' },
        currentInning: 1,
      };
  ...
  describe('getTeamLineup', () => {
    it('should retrieve team lineup successfully and transform domain data to UI format', async () => {
      // Mock game with team lineup
      const mockGame = {
        id: { value: 'game-123' },
        getHomeTeam: vi.fn().mockReturnValue({
          teamLineupId: { value: 'home-lineup-123' },
          teamName: 'Eagles',
          teamSide: 'HOME',
          strategy: 'DETAILED',
          getActiveLineup: vi.fn().mockReturnValue([
          ...
          getFieldingPositions: vi.fn().mockReturnValue(
            new Map([
              [
                'SHORTSTOP',
                {
                  ...
                  equals: vi
                    .fn()
                    .mockImplementation(
                      (other: { value: string } | undefined) => other?.value === 'player-1'
                    ),
                },
              ],
              [
                'SECOND_BASE',
                {
                  ...
                  equals: vi
                    .fn()
                    .mockImplementation(
                      (other: { value: string } | undefined) => other?.value === 'player-2'
                    ),
                },
              ],
            ])
          ...
          fieldPositions: {
            SHORTSTOP: { value: 'player-1' },
            SECOND_BASE: { value: 'player-2' },
            FIRST_BASE: null,
            THIRD_BASE: null,
            CATCHER: null,
            PITCHER: null,
        ...
        getCurrentInning: vi.fn().mockReturnValue(3),
        isTopHalf: vi.fn().mockReturnValue(false),
      };
      ...
      (
        mockTeamLineupRepository.findByGameIdAndSide as MockedFunction<
          typeof mockTeamLineupRepository.findByGameIdAndSide
        >
      ).mockResolvedValue(mockGame.getHomeTeam() as unknown);

      const uiData = { gameId: 'game-123' };
      const result = await gameAdapter.getTeamLineup(uiData);

      expect(mockTeamLineupRepository.findByGameIdAndSide).toHaveBeenCalledWith(
        expect.objectContaining({ value: 'game-123' }),
        'HOME'
      );

      expect(result).toEqual({
        success: true,
        gameId: expect.objectContaining({ value: 'game-123' }),
        activeLineup: [
          { battingSlot: 1, playerId: 'player-1', fieldPosition: 'SHORTSTOP' },
          { battingSlot: 2, playerId: 'player-2', fieldPosition: 'SECOND_BASE' },
        ],
        benchPlayers: [],
        substitutionHistory: [],
      });
    ...
    it('should handle game not found error', async () => {
      ...
      const uiData = { gameId: 'nonexistent-game' };

      await expect(gameAdapter.getTeamLineup(uiData)).rejects.toThrow('Game not found');
    ...
    it('should validate required gameId', async () => {
      ...
      await expect(gameAdapter.getTeamLineup(uiData)).rejects.toThrow(
    ...
    it('should handle repository errors gracefully', async () => {
      ...
      await expect(gameAdapter.getTeamLineup(uiData)).rejects.toThrow('Database connection failed');
    ...
    it('should transform substituted starters correctly', async () => {
      const mockGame = {
        id: { value: 'game-456' },
        getHomeTeam: vi.fn().mockReturnValue({
          teamLineupId: { value: 'home-lineup-456' },
          teamName: 'Hawks',
          teamSide: 'HOME',
          strategy: 'DETAILED',
          getActiveLineup: vi.fn().mockReturnValue([
              ...
              [
                'SHORTSTOP',
                {
                  ...
                  equals: vi
                    .fn()
                    .mockImplementation(
                      (other: { value: string } | undefined) => other?.value === 'sub-1'
                    ),
                },
              ],
            ])
          ...
          battingSlots: [
            {
              slotNumber: 1,
              currentPlayer: {
                playerId: { value: 'sub-1' },
                name: 'Substitute Player',
                jerseyNumber: { value: '99' },
                fieldPosition: 'SHORTSTOP',
                isActive: true,
                entryInning: 3,
                exitInning: null,
              ...
              history: [
                {
                  playerId: { value: 'starter-1' },
                  playerName: 'Original Starter',
                  enteredInning: 1,
                  exitedInning: 3,
                  wasStarter: true,
                  isReentry: false,
          ...
          substitutionHistory: [
            {
              incomingPlayerId: { value: 'sub-1' },
              outgoingPlayerId: { value: 'starter-1' },
              incomingPlayerName: 'Substitute Player',
              outgoingPlayerName: 'Original Starter',
              battingSlot: 1,
              inning: 3,
              isReentry: false,
              timestamp: new Date('2023-07-15T15:30:00Z'),
            },
          ],
        }),
        getCurrentInning: vi.fn().mockReturnValue(4),
        isTopHalf: vi.fn().mockReturnValue(true),
      };
      ...
      const uiData = { gameId: 'game-456' };
      const result = await gameAdapter.getTeamLineup(uiData);

      expect(result).toEqual({
        success: true,
        gameId: expect.objectContaining({ value: 'game-456' }),
        activeLineup: [{ battingSlot: 1, playerId: 'sub-1', fieldPosition: 'SHORTSTOP' }],
        benchPlayers: [],
        substitutionHistory: [],
      });
    });

    it('should handle games with empty bench correctly', async () => {
      const mockGame = {
        id: { value: 'game-789' },
        getHomeTeam: vi.fn().mockReturnValue({
          teamLineupId: { value: 'home-lineup-789' },
        ...
        getCurrentInning: vi.fn().mockReturnValue(1),
        isTopHalf: vi.fn().mockReturnValue(true),
      };
      ...
      const uiData = { gameId: 'game-789' };
      const result = await gameAdapter.getTeamLineup(uiData);

      expect(result).toEqual({
        success: true,
        gameId: expect.objectContaining({ value: 'game-789' }),
        activeLineup: [{ battingSlot: 1, playerId: 'player-1', fieldPosition: 'PITCHER' }],
        benchPlayers: [],
        substitutionHistory: [],
      });
    });
  ...
  describe('makeSubstitution', () => {
    it('should make substitution successfully with proper validation', async () => {
      ...
      (
        mockSubstitutePlayer.execute as MockedFunction<typeof mockSubstitutePlayer.execute>
      ).mockResolvedValue(mockSubstitutionResult);

      // Mock game for getting current state
      const mockGame = {
        id: { value: 'game-123' },
        currentInning: 5,
      };

      // Mock team lineup for finding players
      const mockTeamLineup: Record<string, unknown> = {
        id: { value: 'home-lineup-123', equals: vi.fn().mockReturnValue(true) },
        getPlayerInfo: vi.fn().mockImplementation((playerId: { value: string }) => {
          if (playerId.value === 'player-1') {
            return {
              playerId: { value: 'player-1' },
          ...
          if (playerId.value === 'bench-1') {
            return {
              playerId: { value: 'bench-1' },
              playerName: 'Incoming Player',
              jerseyNumber: { value: '20', toNumber: () => 20 },
              isStarter: false,
              hasUsedReentry: false,
            };
          }
      ...
      const uiData = {
        ...
        battingSlot: 3,
        fieldPosition: 'THIRD_BASE' as unknown,
        isReentry: false,
      };

      const result = await gameAdapter.makeSubstitution(uiData);

      expect(mockSubstitutePlayer.execute).toHaveBeenCalledWith(
        expect.objectContaining({
          gameId: expect.objectContaining({ value: 'game-123' }),
          outgoingPlayerId: expect.objectContaining({ value: 'player-1' }),
          incomingPlayerId: expect.objectContaining({ value: 'bench-1' }),
          newFieldPosition: 'THIRD_BASE',
        })
      );
      ...
      expect(result).toEqual({
        ...
        substitution: {
          ...
          incomingPlayer: {
            playerId: expect.objectContaining({ value: 'bench-1' }),
    ...
    it('should validate required fields', async () => {
      const invalidData = {
        ...
        incomingPlayerId: 'bench-1',
        battingSlot: 1,
        fieldPosition: 'SHORTSTOP' as unknown,
        isReentry: false,
      };
    ...
    it('should handle re-entry substitutions correctly', async () => {
      const mockSubstitutionResult = {
        success: true,
        gameState: {
          gameId: { value: 'game-123' },
      ...
      const mockTeamLineup: Record<string, unknown> = {
        id: { value: 'home-lineup-123', equals: vi.fn().mockReturnValue(true) },
        getPlayerInfo: vi.fn().mockImplementation((playerId: { value: string }) => {
          if (playerId.value === 'sub-player') {
            return {
              playerId: { value: 'sub-player' },
              playerName: 'Substitute Player',
              jerseyNumber: { value: '99', toNumber: () => 99 },
              isStarter: false,
              hasUsedReentry: false,
            };
          }
          if (playerId.value === 'original-starter') {
            return {
              playerId: { value: 'original-starter' },
              playerName: 'Original Starter',
              jerseyNumber: { value: '7', toNumber: () => 7 },
              isStarter: true,
              hasUsedReentry: false,
            };
          }
        ...
        getActiveLineup: vi.fn().mockReturnValue([
          {
            ...
            currentPlayer: {
              ...
              equals: vi
                .fn()
                .mockImplementation((other: { value: string }) => other.value === 'sub-player'),
            },
          },
        ]),
      };
      ...
      const uiData = {
        gameId: 'game-123',
        outgoingPlayerId: 'sub-player',
        incomingPlayerId: 'original-starter',
        battingSlot: 1,
        fieldPosition: 'SHORTSTOP' as unknown,
        isReentry: true,
      };
      ...
      expect(result).toEqual({
        ...
        substitution: {
          ...
          battingSlot: 1,
          outgoingPlayer: {
            playerId: expect.objectContaining({ value: 'sub-player' }),
            name: 'Substitute Player',
          },
          incomingPlayer: {
            playerId: expect.objectContaining({ value: 'original-starter' }),
            name: 'Original Starter',
          },
          timestamp: expect.any(Date),
          isReentry: true,
    ...
    it('should handle substitution execution errors', async () => {
      // Mock game repository to return a valid game
      const mockGame = {
        id: { value: 'game-123' },
        currentInning: 5,
      };
      ...
      // Mock team lineup with proper player data
      const mockTeamLineup = {
        id: { value: 'home-lineup-123', equals: vi.fn().mockReturnValue(true) },
        getPlayerInfo: vi.fn().mockImplementation((playerId: { value: string }) => {
          if (playerId.value === 'player-1') {
      ...
      const substitutionError = new Error('Player not eligible for substitution');
      (
        mockSubstitutePlayer.execute as MockedFunction<typeof mockSubstitutePlayer.execute>
      ).mockRejectedValue(substitutionError);
      ...
      await expect(gameAdapter.makeSubstitution(uiData)).rejects.toThrow(
        'Player not eligible for substitution'
      );
    ...
    it('should handle missing game during substitution', async () => {
      (
        mockGameRepository.findById as MockedFunction<typeof mockGameRepository.findById>
      ).mockResolvedValue(null);
      ...
      await expect(gameAdapter.makeSubstitution(uiData)).rejects.toThrow('Game not found');
    });

    it('should validate battingSlot type', async () => {
      const invalidData = {
        gameId: 'game-123',
        outgoingPlayerId: 'player-1',
        incomingPlayerId: 'bench-1',
        battingSlot: 'invalid' as unknown,
        fieldPosition: 'SHORTSTOP' as unknown,
        isReentry: false,
      };
