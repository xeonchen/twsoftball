...
describe('InningState - Core Operations', () => {
  ...
  describe('recordAtBat', () => {
    ...
    it('should record a triple and advance batter to third base', () => {
      ...
      const updated = inningState.recordAtBat(batterId, 1, AtBatResultType.TRIPLE, 1, undefined, [
        { runnerId: batterId, from: null, to: 'THIRD' },
      ]);
    ...
    it('should emit RunnerAdvanced to HOME for home runs (RunScored generated by Application layer)', () => {
      const inningState = InningState.createNew(inningStateId, gameId);
      ...
      expect(runnerAdvancedToHomeEvents[0]).toMatchObject({
        gameId,
        runnerId: batterId,
        from: null, // Batter starting from home plate
        to: 'HOME',
      });
    });
  ...
  describe('advanceRunners', () => {
    ...
    it('should emit RunnerAdvanced to HOME when runners score (RunScored generated by Application layer)', () => {
      const runner1 = new PlayerId('runner-1');
      const inningState = InningState.createNew(inningStateId, gameId).withRunnerOnBase(
        'THIRD',
        runner1
  ...
  describe('helper methods for test setup', () => {
    ...
    describe('getBases', () => {
      ...
      it('should return runner on first base', () => {
        ...
        const updated = inningState.recordAtBat(runner, 1, AtBatResultType.SINGLE, 1, undefined, [
          { runnerId: runner, from: null, to: 'FIRST' },
        ]);
  ...
  describe('recordAtBat - walk-off scenarios', () => {
    it('should NOT advance inning when walk-off detected (bottom 7, 2 outs, home takes lead)', () => {
      // Setup: Bottom 7, 2 outs, home trailing 4-5
      const inningState = InningState.createNew(inningStateId, gameId)
        .withInningHalf(7, false) // Bottom 7
        .withOuts(2); // 2 outs
      ...
      const gameContext = {
        ...
        awayScore: 5,
        totalInnings: 7,
        runsAboutToScore: 2, // Home run scores 2 runs (tie would be 5-5, but batter scoring makes it 6-5)
      };

      // At-bat: Homerun (batter scores, making final score 6-5 - home takes lead)
      const updated = inningState.recordAtBat(
        batterId,
        1,
      ...
      expect(updated.inning).toBe(7); // Should stay at 7
      expect(updated.isTopHalf).toBe(false); // Should still be bottom
      expect(updated.outs).toBe(2); // Outs should not reset (no HalfInningEnded)

      // Verify NO HalfInningEnded event was emitted
      const events = updated.getUncommittedEvents();
      const halfInningEndedEvents = events.filter(e => e.type === 'HalfInningEnded');
      expect(halfInningEndedEvents).toHaveLength(0);
    });

    it('should advance inning normally when NOT walk-off (top half of final inning)', () => {
      // Setup: Top 7, 2 outs
      const inningState = InningState.createNew(inningStateId, gameId)
        .withInningHalf(7, true) // Top 7
        .withOuts(2);
      ...
      const gameContext = {
        ...
        awayScore: 5,
        totalInnings: 7,
        runsAboutToScore: 2, // Away team scores 2 runs (7-4)
      };

      // Record 3rd out in top half
      const updated = inningState.recordAtBat(batterId, 1, AtBatResultType.FLY_OUT, 7, gameContext);

      // NOT walk-off (top half) - should transition to bottom 7
      expect(updated.inning).toBe(7);
      expect(updated.isTopHalf).toBe(false); // Flipped to bottom
      expect(updated.outs).toBe(0); // Reset to 0

      // Verify HalfInningEnded event WAS emitted
      const events = updated.getUncommittedEvents();
      const halfInningEndedEvents = events.filter(e => e.type === 'HalfInningEnded');
      expect(halfInningEndedEvents).toHaveLength(1);
    });

    it('should advance inning normally when NOT walk-off (bottom of non-final inning)', () => {
      // Setup: Bottom 6, 2 outs (not final inning yet)
      const inningState = InningState.createNew(inningStateId, gameId)
        .withInningHalf(6, false) // Bottom 6
        .withOuts(2);
      ...
      const gameContext = {
        ...
        awayScore: 3,
        totalInnings: 7,
        runsAboutToScore: 2, // Home team scores 2 runs (6-3)
      };
      ...
      // NOT walk-off (too early) - should advance to top 7
      expect(updated.inning).toBe(7); // Advanced to next inning
      expect(updated.isTopHalf).toBe(true); // Flipped to top
      expect(updated.outs).toBe(0); // Reset to 0

      // Verify both HalfInningEnded and InningAdvanced events
      const events = updated.getUncommittedEvents();
      const halfInningEndedEvents = events.filter(e => e.type === 'HalfInningEnded');
      const inningAdvancedEvents = events.filter(e => e.type === 'InningAdvanced');
      expect(halfInningEndedEvents).toHaveLength(1);
      expect(inningAdvancedEvents).toHaveLength(1);
    });

    it('should NOT trigger walk-off when home team only ties (not winning)', () => {
      // Setup: Bottom 7, 2 outs, home trailing 4-5
      const inningState = InningState.createNew(inningStateId, gameId)
        .withInningHalf(7, false) // Bottom 7
        .withOuts(2);
      ...
      const gameContext = {
        ...
        awayScore: 5,
        totalInnings: 7,
        runsAboutToScore: 1, // Home scores 1 run (ties 5-5, but doesn't take lead)
      };

      // Record at-bat that ties the game
      const updated = inningState.recordAtBat(batterId, 1, AtBatResultType.SINGLE, 7, gameContext);

      // NOT walk-off (only tied, not winning) - should continue normal play
      // With 2 outs already, this should NOT end the inning (no 3rd out)
      expect(updated.outs).toBe(2); // Still 2 outs
      expect(updated.inning).toBe(7);
      expect(updated.isTopHalf).toBe(false);
    });

    it('should trigger walk-off in extra innings (bottom 8)', () => {
      // Setup: Bottom 8 (extra innings), 1 out, home trailing 5-6
      const inningState = InningState.createNew(inningStateId, gameId)
        .withInningHalf(8, false) // Bottom 8
        .withOuts(1);

      const gameContext = {
        homeScore: 5,
        awayScore: 6,
        totalInnings: 7, // Standard game
        runsAboutToScore: 2, // Home scores 2 runs (7-6 - takes lead)
      };

      // Record walk-off hit
      const updated = inningState.recordAtBat(batterId, 1, AtBatResultType.DOUBLE, 8, gameContext);

      // Walk-off in extra innings - inning should NOT advance
      expect(updated.inning).toBe(8);
      expect(updated.isTopHalf).toBe(false);
      expect(updated.outs).toBe(1); // Outs should not change

      // No HalfInningEnded event
      const events = updated.getUncommittedEvents();
      const halfInningEndedEvents = events.filter(e => e.type === 'HalfInningEnded');
      expect(halfInningEndedEvents).toHaveLength(0);
    });

    it('should work with optional gameContext parameter (backward compatibility)', () => {
      // Setup: Bottom 7, 2 outs
      const inningState = InningState.createNew(inningStateId, gameId)
        .withInningHalf(7, false)
        .withOuts(2);

      // Call recordAtBat WITHOUT gameContext (should still work, no walk-off detection)
      const updated = inningState.recordAtBat(batterId, 1, AtBatResultType.FLY_OUT, 7);

      // Normal advancement (no walk-off detection without context)
      // Bottom 7 ends with 3rd out, advances to Top 8
      expect(updated.inning).toBe(8); // Advances to next inning
      expect(updated.isTopHalf).toBe(true); // Flips to top half
      expect(updated.outs).toBe(0); // 3rd out resets to 0

      // Both HalfInningEnded and InningAdvanced should be emitted (normal flow)
      const events = updated.getUncommittedEvents();
      const halfInningEndedEvents = events.filter(e => e.type === 'HalfInningEnded');
      const inningAdvancedEvents = events.filter(e => e.type === 'InningAdvanced');
      expect(halfInningEndedEvents).toHaveLength(1);
    ...
    it('should require home team to be WINNING for walk-off (not just leading before the play)', () => {
      // Setup: Bottom 7, 0 outs, home already leading 6-5
      const inningState = InningState.createNew(inningStateId, gameId)
        .withInningHalf(7, false)
        .withOuts(0);
      ...
      const gameContext = {
        ...
        awayScore: 5,
        totalInnings: 7,
        runsAboutToScore: 1, // Home scores another run (7-5)
      };

      // Record hit (home already winning, scores another run)
      const updated = inningState.recordAtBat(batterId, 1, AtBatResultType.SINGLE, 7, gameContext);

      // Walk-off detected (home winning after this play)
      expect(updated.inning).toBe(7);
      expect(updated.isTopHalf).toBe(false);
      expect(updated.outs).toBe(0); // No outs recorded

      // No HalfInningEnded (walk-off ends game immediately)
      const events = updated.getUncommittedEvents();
      const halfInningEndedEvents = events.filter(e => e.type === 'HalfInningEnded');
      expect(halfInningEndedEvents).toHaveLength(0);
    });

    it('should NOT trigger walk-off if no runs score on the play', () => {
      // Setup: Bottom 7, 0 outs, home trailing 4-5
      const inningState = InningState.createNew(inningStateId, gameId)
        .withInningHalf(7, false)
        .withOuts(0);
      ...
      const gameContext = {
        ...
        awayScore: 5,
        totalInnings: 7,
        runsAboutToScore: 0, // No runs score (e.g., single with no one on base)
      };

      // Record single with no runs
      const updated = inningState.recordAtBat(batterId, 1, AtBatResultType.SINGLE, 7, gameContext);

      // NOT walk-off (no runs scored) - normal play continues
      expect(updated.inning).toBe(7);
      expect(updated.isTopHalf).toBe(false);
      expect(updated.outs).toBe(0); // No change

      // No events related to inning end
      const events = updated.getUncommittedEvents();
      const halfInningEndedEvents = events.filter(e => e.type === 'HalfInningEnded');
      expect(halfInningEndedEvents).toHaveLength(0);
    });
